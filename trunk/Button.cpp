...
Button::Button(const std::string& txt, int x, int y)
{
    ...
    buttonSprite.SetPosition((WIDTH+200)/2 - buttonSprite.GetGlobalBounds().Width/2, y);
    highlighted = false;
    text = rm.getText(txt, 15);
    sf::FloatRect rect = text.GetGlobalBounds();
    text.SetPosition((WIDTH+200)/2 - rect.Width/2 , y + buttonSprite.GetGlobalBounds().Height/2 - rect.Height/2);
}
